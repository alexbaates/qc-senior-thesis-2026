...
import scipy
from .VQAs import VQE_qulacs as vc
...
class CircuitEnv():
    ...
    def __init__(self, conf, device, circuit_path=None, hamiltonian_path=None):
        ...
        if self.TN_bond:
            if circuit_path is not None:
                _circ_path = circuit_path
            elif self.ham_type not in ['heisenberg', 'tfim_j1_h0.05']:
                _circ_path = f"dmrg-to-qc/init_state_circ/init_{self.ham_type}_{self.num_qubits}q_geom_{self.geometry}_{self.ham_mapping}_TNbond{self.TN_bond}.qpy"
            else:
                _circ_path = f"dmrg-to-qc/init_state_circ/init_{self.ham_type}_{self.num_qubits}q_TNbond{self.TN_bond}.qpy"
            with open(_circ_path, "rb") as qpy_file_read:
        ...
        self._hamiltonian_path = hamiltonian_path
        if hamiltonian_path is not None:
            __ham = np.load(hamiltonian_path)
        elif self.ham_type not in ['heisenberg', 'tfim_j1_h0.05']:
    ...
    def reset(self):
        ...
        if self.TN_init:
            def _qarg_index(qarg):
                # Prefer direct index attributes and fall back to repr parsing.
                if hasattr(qarg, '_index') and qarg._index is not None:
                    return int(qarg._index)
                if hasattr(qarg, 'index') and isinstance(qarg.index, int):
                    return int(qarg.index)
                match = re.search(r'index=(\d+)', str(qarg))
                if match is not None:
                    return int(match.group(1))
                raise ValueError(f"Unable to parse qubit index from qarg: {qarg}")
            ...
            for depth_no, circ in enumerate(self.depth_wise_gates):
                ...
                for c in circ:
                    ...
                    if c.op.name in ('rx', 'ry', 'rz'):
                        gate_pos = _qarg_index(c.qargs[0])
                        ...
                        ctrl = _qarg_index(c.qargs[0])
                        targ = _qarg_index(c.qargs[1])
        ...
        if self._hamiltonian_path is not None:
            __ham = np.load(self._hamiltonian_path)
        elif self.ham_type not in ['heisenberg', 'tfim_j1_h0.05']:
            __ham = np.load(f"dmrg-to-qc/mol_data/{self.ham_type}_{self.num_qubits}q_geom_{self.geometry}_{self.ham_mapping}.npz")
        else:
            __ham = np.load(f"dmrg-to-qc/mol_data/{self.ham_type}_{self.num_qubits}q.npz")
